...
use super::pure_globals::{
  CalleePosition, PureGlobalArgs, classify_pure_global, is_pure_global_access,
};
use crate::{
...
fn is_pure_global_arg_shape(
  ...
  match arg_rule {
    ...
    PureGlobalArgs::ArrayOrNumericLength => is_safe_array_or_numeric_length(args, unresolved_ctxt),
    PureGlobalArgs::ArrayIterableOrEmpty => args
      .first()
      .is_none_or(|arg| matches!(arg.expr.as_ref(), Expr::Array(_))),
  }
}
...
fn is_safe_array_single_arg(expr: &Expr, unresolved_ctxt: SyntaxContext) -> bool {
  if let Some(value) = static_number_value(expr, unresolved_ctxt) {
    return is_terser_array_range(value);
  }

  false
}

fn is_safe_array_or_numeric_length(args: &[ExprOrSpread], unresolved_ctxt: SyntaxContext) -> bool {
  args.first().is_none_or(|arg| {
    matches!(arg.expr.as_ref(), Expr::Array(_))
      || static_number_value(&arg.expr, unresolved_ctxt).is_some_and(is_terser_array_range)
  })
}

fn is_terser_array_range(value: f64) -> bool {
  value >= 0.0 && value <= 4_294_967_295.0
}
...
pub fn is_pure_expression<'a>(
  ...
  pub fn _is_pure_expression<'a>(
    ...
    match expr {
      ...
      _ => {
        if builtin_pure_globals && is_pure_global_access(expr, unresolved_ctxt) {
          return true;
        }
