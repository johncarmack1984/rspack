  ...
  pub fn to_path_data<'a>(
    ...
    Ok(rspack_core::PathData {
      ...
      chunk_name: chunk
        ...
        .or(self.chunk_name.as_deref()),
      chunk_hash: chunk_hash.or(self.chunk_hash.as_deref()),
      chunk_id: chunk
  ...
  unsafe fn from_napi_value(env: sys::napi_env, napi_val: sys::napi_value) -> napi::Result<Self> {
    unsafe {
      ...
      Ok(JsPathData {
        ...
        chunk: get_chunk(&object)?,
        chunk_name: get_chunk_field::<String>(&object, "name")?,
        chunk_hash: get_chunk_field::<String>(&object, "hash")?,
        chunk_id: get_chunk_field::<JsPathDataId>(&object, "id")?.map(id_to_string),
      })
    }
  }
}

fn get_chunk_field<T: FromNapiValue>(object: &Object, key: &str) -> napi::Result<Option<T>> {
  let Some(value) = object.get::<Unknown>("chunk")? else {
    return Ok(None);
  };
  let Ok(chunk) = (unsafe { value.cast::<Object>() }) else {
    return Ok(None);
  };
  chunk.get::<T>(key)
}

fn get_chunk(object: &Object) -> napi::Result<Option<ChunkWrapper>> {
  let Some(value) = object.get::<Unknown>("chunk")? else {
    return Ok(None);
  };

  match unsafe { value.cast::<ChunkWrapper>() } {
