...
:::info
除了 `process.env.KEY` 之外，Rspack 还会把每个 key 暴露为 `import.meta.env.KEY`，因此同样的变量也可以在 ESM 源码中访问。参见 [DefinePlugin — `import.meta.env`](/plugins/webpack/define-plugin#import-meta-env)。
:::
...
使用 [`dotenv`](/config/dotenv) 配置项可以从 `.env` 文件加载环境变量。每个匹配到的变量会同时暴露为 `import.meta.env.KEY` 和 `process.env.KEY`：
...
前缀、目录和模板等选项见 [`dotenv`](/config/dotenv)。
