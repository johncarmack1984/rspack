...
:::info
In addition to `process.env.KEY`, Rspack also exposes every key as `import.meta.env.KEY`, so the same variables are reachable from ESM source code. See [DefinePlugin — `import.meta.env`](/plugins/webpack/define-plugin#import-meta-env).
:::
...
Use the [`dotenv`](/config/dotenv) config option to load environment variables from `.env` files. Every matching variable is exposed as both `import.meta.env.KEY` and `process.env.KEY`:
...
See [`dotenv`](/config/dotenv) for prefix, directory and template options.
